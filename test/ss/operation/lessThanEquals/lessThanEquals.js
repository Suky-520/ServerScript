...
assert.equal('2' <= '12', false); // 字符串比较，'2' 在字典顺序上大于 '1'
...
let b={}
assert.equal(b<= 0, false); // 对象 {} 无法转换为一个有意义的数值进行比较
...
assert.equal(b <= 0, false); // 对象与数值比较通常返回 false 除非对象定义了 valueOf 或 toString 方法
